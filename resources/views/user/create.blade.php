<x-app-layout>
    <div class="max-w-lg">
        <div class="mb-6">
            ...
            <p class="text-sm text-slate-400 mt-0.5">Fill in the details to create a new account</p>
        </div>

        <form action="{{ route('users.store') }}" method="POST" class="bg-white rounded-2xl p-6 flex flex-col gap-4" style="border:1px solid #e2e8f0;">
            @csrf

            {{-- Name --}}
            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1.5">Full Name</label>
                <input name="name" type="text" placeholder="e.g. John Doe" value="{{ old('name') }}" class="w-full px-3.5 py-2.5 rounded-xl text-sm text-slate-700 outline-none transition-all @error('name') border-red-400 bg-red-50 @else border-slate-200 bg-slate-50 @enderror" style="border-width:1px;" onfocus="this.style.borderColor='#2563eb';this.style.boxShadow='0 0 0 3px rgba(37,99,235,0.1)'" onblur="this.style.borderColor='';this.style.boxShadow='none'">
                @error('name')
            ...
            <div>
                ...
                <input name="email" type="email" placeholder="e.g. john@example.com" value="{{ old('email') }}" class="w-full px-3.5 py-2.5 rounded-xl text-sm text-slate-700 outline-none transition-all @error('email') border-red-400 bg-red-50 @else border-slate-200 bg-slate-50 @enderror" style="border-width:1px;" onfocus="this.style.borderColor='#2563eb';this.style.boxShadow='0 0 0 3px rgba(37,99,235,0.1)'" onblur="this.style.borderColor='';this.style.boxShadow='none'">
                @error('email')
                    <p class="text-xs text-red-500 mt-1.5 flex items-center gap-1">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <circle cx="12" cy="12" r="10"></circle>
