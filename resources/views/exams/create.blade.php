<x-app-layout>
    <div class="max-w-3xl">
        <div class="mb-6">
            <h1 class="text-xl font-bold text-slate-800">Create Exam</h1>
            <p class="text-sm text-slate-400 mt-0.5">Fill in exam details and add questions</p>
        </div>

        @php
            info('asdfasdf');
            if ($errors->any()) {
                info(json_encode($errors->all()));
            }
        @endphp
            ...
            <div class="bg-white rounded-2xl p-6 flex flex-col gap-4 mb-4" style="border:1px solid #e2e8f0;">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2">
                        ...
                        <input name="name" type="text" placeholder="e.g. Mid-Year Mathematics Exam" value="{{ old('name') }}" class="w-full px-3.5 py-2.5 rounded-xl text-sm text-slate-700 outline-none @error('name') border-red-400 bg-red-50 @else border-slate-200 bg-slate-50 @enderror" style="border-width:1px;" onfocus="this.style.borderColor='#2563eb';this.style.boxShadow='0 0 0 3px rgba(37,99,235,0.1)'" onblur="this.style.borderColor='';this.style.boxShadow='none'">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input name="duration" type="number" min="1" placeholder="e.g. 60" value="{{ old('duration') }}" class="w-full px-3.5 py-2.5 rounded-xl text-sm text-slate-700 outline-none border-slate-200 bg-slate-50" style="border-width:1px;" onfocus="this.style.borderColor='#2563eb';this.style.boxShadow='0 0 0 3px rgba(37,99,235,0.1)'" onblur="this.style.borderColor='';this.style.boxShadow='none'">
                        @error('duration')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input name="start" type="datetime-local" value="{{ old('start') }}" class="w-full px-3.5 py-2.5 rounded-xl text-sm text-slate-700 outline-none border-slate-200 bg-slate-50" style="border-width:1px;" onfocus="this.style.borderColor='#2563eb';this.style.boxShadow='0 0 0 3px rgba(37,99,235,0.1)'" onblur="this.style.borderColor='';this.style.boxShadow='none'">
                        @error('start')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input name="end" type="datetime-local" value="{{ old('end') }}" class="w-full px-3.5 py-2.5 rounded-xl text-sm text-slate-700 outline-none border-slate-200 bg-slate-50" style="border-width:1px;" onfocus="this.style.borderColor='#2563eb';this.style.boxShadow='0 0 0 3px rgba(37,99,235,0.1)'" onblur="this.style.borderColor='';this.style.boxShadow='none'">
                        @error('end')
                            <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="description" rows="2" placeholder="Brief description of this exam..." class="w-full px-3.5 py-2.5 rounded-xl text-sm text-slate-700 outline-none border-slate-200 bg-slate-50 resize-none" style="border-width:1px;" onfocus="this.style.borderColor='#2563eb';this.style.boxShadow='0 0 0 3px rgba(37,99,235,0.1)'" onblur="this.style.borderColor='';this.style.boxShadow='none'">{{ old('description') }}</textarea>
                    </div>
                        ...
                        <div class="flex flex-col gap-1 overflow-y-auto rounded-xl p-2" style="max-height:120px;border:1px solid #e2e8f0;background:#fafafa;">
                            @foreach ($classes as $class)
                                <label class="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-blue-50 cursor-pointer transition-colors">
                                    <input type="checkbox" name="class_ids[]" value="{{ $class->id }}" class="w-4 h-4 rounded accent-blue-600" {{ in_array($class->id, old('class_ids', [])) ? 'checked' : '' }}>
            ...
            <div class="bg-white rounded-2xl p-6 mb-4" style="border:1px solid #e2e8f0;">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex gap-2">
                        <button type="button" onclick="addQuestion('mcq')" class="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-colors hover:opacity-90" style="background:#eff6ff;color:#2563eb;">
                        ...
                        <button type="button" onclick="addQuestion('open_text')" class="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-colors hover:opacity-90" style="background:#f0fdf4;color:#16a34a;">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                <line x1="12" y1="5" x2="12" y2="19"></line>
                                <line x1="5" y1="12" x2="19" y2="12"></line>
                            </svg>
                ...
                <div id="questions-container" class="flex flex-col gap-4">
                    <p id="empty-msg" class="text-sm text-slate-400 text-center py-8">No questions yet. Click the buttons above to add questions.</p>
                </div>
            </div>
    ...
    <script>
        ...
        function addQuestion(type) {
            ...
            const color = type === 'mcq' ? '#eff6ff|#2563eb' : '#f0fdf4|#16a34a';
    ...
    `;
        ...
        function removeQuestion(idx) {
            ...
            if (!document.getElementById('questions-container').children.length) {
                ...
                p.textContent = 'No questions yet. Click the buttons above to add questions.';
